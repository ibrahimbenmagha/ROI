...
const CreateActivity = () => {
  ...
  // Normalize item name for "Coût Total"
  const normalizeItemName = (name) => {
    if (name && typeof name === "string" && /coût\s*total/i.test(name)||/coût\s*Total/i.test(name) || /Coût\s*Total/i.test(name)) {
      return "i";
    }
    return name;
  };
  ...
  //         name: normalizeItemName(item.name), // Normalize item name
  ...
  //     const response = await axiosInstance.post("createActivity", payload);
...
// Update the handleSubmit function to use 'formula' instead of 'formulat'
const handleSubmit = async (e) => {
  e.preventDefault();
  setSubmitting(true);
  try {
    ...
    const payload = {
      ...
      formula: formulat, // Changed from 'formulat' to 'formula'
    };
  ...
  const openCalculatorModal = () => {
    if (
      items.length === 0 ||
      items.some((item) => !item.name || !item.symbole)
    ) {
      message.error(
  ...
  const addExpressionElement = (type, value) => {
    ...
    const canAddOperator =
      ...
      (newFormulaExpression[newFormulaExpression.length - 1].type === "item" ||
        newFormulaExpression[newFormulaExpression.length - 1].type ===
          "constant" ||
        newFormulaExpression[newFormulaExpression.length - 1].type ===
          "formula");

    if (
      (type === "item" || type === "constant" || type === "formula") &&
      !canAddItemOrConstant
    ) {
      message.error(
  ...
  const handleAddFormula = () => {
    ...
    const formulaKey = newFormulaKey.toLowerCase().includes("roi")
      ? "roi"
      : newFormulaKey;
    setFormulat({
      ...formulat,
  ...
  const parseExpression = (expressionStr) => {
    ...
    return parts.map((part) => {
      if (items.some((i) => i.symbole === part))
        return { type: "item", value: part };
      if (Object.keys(formulat).includes(part))
        return { type: "formula", value: part };
