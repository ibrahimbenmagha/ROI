...
const ActivityPage: React.FC = () => {
  ...
  const handleExportCsv = async () => {
    try {
      const response = await axiosInstance.get("/exportAllActivitiesCsv", {
      ...
      link.href = url;
      link.setAttribute("download", `export_activities_labo.csv`); // Nom du fichier
      ...
      message.error(
        error.response?.data?.message || "Échec de l'exportation CSV."
      );
    }
  };
      ...
      <div className="w-full px-4 md:px-10 pb-6 mt-8">
        <div className="flex flex-wrap justify-end gap-4">
          ...
          <Button
            ...
            type="default"
            onClick={() => message.info("Fonction Export PDF à implémenter")}
          >
            Exporter PDF
          </Button>
          <Upload
