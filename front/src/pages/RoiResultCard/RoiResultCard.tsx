...
//               <CardDescription>
//                 {activityData
//                   ? `Année: ${activityData.activityByLabo.year}`
...
import React, { useState, useEffect } from "react";
import { jsPDF } from "jspdf";
import { Layout, Typography, Spin, Empty, message, Modal, Form, Input, InputNumber } from "antd";
import { useNavigate } from "react-router-dom";
import { Button } from "@/components/ui/button";
import {
  Card,
...
const DisplayCalculatedData = () => {
  ...
  const handleExportCsv = () => {
    ...
    const convertToCSV = (data: ActivityData) => {
      const header = "Section,Clé,Valeur\n";
      const activityByLaboRows = Object.entries(data.activityByLabo)
        .map(([key, value]) => `"activityByLabo","${key.replace(/"/g, '""')}","${value}"`)
        .join("\n");
      const itemsRows = data.items
        .map(
          (item) =>
            `"items","${item.name.replace(/"/g, '""')}","${
              item.value !== null
      ...
      const calculatedRows = Object.entries(data.calculated_results)
        .map(([key, value]) => {
          ...
          return `"calculated_results","${key.replace(/"/g, '""')}","${formattedValue}"`;
        })
        .join("\n");
  ...
  const deleteActivityValues = async (e: React.MouseEvent) => {
    ...
    if (confirmDelete) {
      try {
        ...
        console.error("Erreur lors de la suppression des données:", error);
        message.error(
          error.response?.data?.error || "Erreur lors de la suppression des données"
        );
      }
    }
  };
  ...
  const handleModifySubmit = async (values: any) => {
    try {
      const payload = {
        year: values.year,
        items: values.items.map((item: { id: number; value: number | null }) => ({
      ...
      const response = await axiosInstance.patch(
        `/api/updateActivityByLaboData/${activityData?.activityByLabo.id}`,
        payload,
        { withCredentials: true }
      );
      ...
      message.error(
        error.response?.data?.error || "Erreur lors de la modification des données"
      );
    }
  };
      ...
      <Content style={{ padding: "32px 24px", background: "#f5f5f5" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <Card className="mb-6 shadow-lg">
            <CardHeader>
              <CardTitle>
                Résultats du calcul ROI {activityData ? `- ${activityData.activityByLabo.activity}` : ""}
              </CardTitle>
              <CardDescription>
                {activityData
                  ? `Année: ${activityData.activityByLabo.year}`
              ...
              {loading ? (
                ...
                <>
                  ...
                  <div>
                    <Title level={4}>Résultats calculés</Title>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {Object.entries(activityData.calculated_results).map(([key, value], index) => (
                        <div
                          key={index}
                          className={`bg-white shadow-md p-4 rounded-md flex flex-col items-start ${
                            key.toLowerCase().includes("roi") ? "border-l-4 border-green-500" : ""
                          }`}
                        >
                          <Text className="font-semibold text-sm">
                            {key
            ...
            {(fields) => (
              <>
                {fields.map((field, index) => {
                  const item = activityData?.items[index];
                  return (
                    <Form.Item
                      key={field.key}
                      label={item?.name}
                      ...
                      rules={[
                        { required: true, message: `Veuillez entrer la valeur pour ${item?.name}` },
                        {
                          type: "number",
                          min: 0,
